...
def move_helper(state, movers):
    ...
    if mover_type == "worker":
        ...
        for neighbor in neighbors:
            ...
            new_state["actions"][-1][0] += ", worker from {} to {}".format(
                pos_old, neighbor
            )
        ...
        for neighbor in neighbors:
            ...
            new_state["actions"][-1][0] += ", hero from {} to {}".format(
                pos_old, neighbor
            )
            result += move_helper(new_state, movers[1:])
        ...
        for neighbor in neighbors:
            present_workers = [
                worker_id
                for worker_id, worker in enumerate(state["workers"])
                if worker == pos_old
            ]
            for num_take_workers in range(len(present_workers) + 1):
                ...
                new_state["actions"][-1][
                    0
                ] += ", mech from {} to {} with {} workers".format(
                    pos_old, neighbor, num_take_workers
                )
                result += move_helper(new_state, movers[1:])
    return result
...
def move(state: dict) -> typing.List[dict]:
    ...
    new_states = []
    movables = (
        [("mech", i) for i in range(len(state_move["mechs"]))]
        + [("worker", i) for i in range(len(state_move["workers"]))]
        + [("hero", 0)]
    )
    ...
    for new_state in new_states:
        for final_state in final_states:
            if (
                new_state["hero"] == final_state["hero"]
                and new_state["mechs"] == final_state["mechs"]
                and new_state["workers"] == final_state["workers"]
            ):
                break
        else:
...
def build(state: dict) -> typing.List[dict]:
    ...
    buildings_left = {"armory", "monument", "tunnel", "mill"} - set(
        state["buildings"].keys()
    )
    ...
    for building in buildings_left:
        for hex in set(state["workers"]):
            ...
            if hex in state["buildings"].values():
                continue
            if building == "mill" and board.board[hex].type not in [
                "grain",
                "metal",
                "oil",
                "wood",
            ...
            if (
                "build" in final_state["recruits"].keys()
                and final_state["popularity"] <= 18
            ):
...
def enlist(state: dict) -> typing.List[dict]:
    ...
    recruits_left = {"upgrade", "deploy", "build", "enlist"} - set(
        state["recruits"].keys()
    )
    bonuses_left = {"money", "power", "popularity", "combat cards"} - set(
        state["recruits"].values()
    )
...
def upgrade(state: dict) -> typing.List[dict]:
    ...
    upgrades_top_left = {
        ...
        "movement",
        "money",
    } - set(state["upgrades top"])
    if len(upgrades_top_left) == 0:
        return []
    upgrades_bottom_left = []
    for upgrade_bottom_type in {"upgrade", "deploy", "build", "enlist"}:
        if (
            state["upgrades bottom"][upgrade_bottom_type]
            < getattr(player_mats.mats[state["player mat"]], upgrade_bottom_type)[2]
        ):
    ...
    for upgrade_top in upgrades_top_left:
        for upgrade_bottom in upgrades_bottom_left:
            ...
            final_state["actions"][-1][1] = "Upgrade {} and {}".format(
                upgrade_top, upgrade_bottom
            )
            final_states.append(final_state)
